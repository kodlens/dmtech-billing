...
export default function DostvCreateEdit( 
    ...
    const submit = async (values: object) => {
        ...
        if (id > 0) {
            try {
                const res = await axios.patch("/admin/pages/banners/" + id, values);
                if (res.data.status === "updated") {
                    modal.info({
                        title: "Updated!",
                        content: <div>Banner successfully updated.</div>,
                        onOk() {
                            router.visit("/admin/pages/banners");
                        },
                    });
                }
            } catch (err: any) {
            ...
            try {
                const res = await axios.post("/admin/pages/banners", values);
    ...
    const uploadProps: UploadProps = {
        name: "upload",
        action: "/admin/pages/temp-upload",
        headers: {
